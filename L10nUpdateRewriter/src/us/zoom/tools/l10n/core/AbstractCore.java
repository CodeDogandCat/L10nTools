...
public class AbstractCore {
	...
	protected Processbar bar = null;

	protected void repeat(Map<String, String> map, String key) {
		if (map.containsKey(key)) {
			System.err.println("repeat :" + key);
		}
	}
				...
				if (em.sheetExist(fileDir, sheetName)) {
					...
					case 0:
						...
						for (int i = 0; i < list0.size(); i++) {
							...
							repeat(map, item0.getWhich(0, null));
					...
					case 1:
						...
						for (int i = 0; i < list.size(); i++) {
							...
							repeat(map, item.getWhich(0, null));
					...
					case 2:
						...
						for (int i = 0; i < list2.size(); i++) {
							...
							repeat(map, item2.getWhich(0, null));
					...
					case 3:
						...
						for (int i = 0; i < list3.size(); i++) {
							...
							repeat(map, item3.getWhich(0, null));
					...
					case 4:
						...
						for (int i = 0; i < list4.size(); i++) {
							...
							repeat(map, item4.getWhich(0, null));
					...
					case 5:
						...
						for (int i = 0; i < list5.size(); i++) {
							...
							repeat(map, item5.getWhich(0, null));
					...
					case 6:
						...
						for (int i = 0; i < list6.size(); i++) {
							...
							repeat(map, item6.getWhich(0, null));
		...
		for (Map.Entry<String, String> entry : tag_default_modified.entrySet()) {
			if (!entry.getValue().equals("")) {
				for (int i = 0; i < 6; i++) {
					...
					if (tempMap != null) {
						if (tempMap.size() != 0 && tempMap.containsKey(entry.getKey().trim())) {
							tempMap.put(entry.getKey().trim(), entry.getValue());
							// mark this is updated
							isUpdated.put(i + 1, 1);
							// System.err.println(index + "###update:" +
							// entry.getKey());
							// index++;
						}
					}
		...
		try {
			...
			if (commandMap.containsKey(Command.FRENCH_RESOURCE_FILE)) {
				...
				if (readValueFromExcel(path_french, tag_french, 1)) {
					...
					isUpdated.put(1, 0);
					// process bar
					bar = new Processbar(tag_french.size(), 30);
				} else {
			...
			if (commandMap.containsKey(Command.JAPANESE_RESOURCE_FILE)) {
				...
				if (readValueFromExcel(path_japanese, tag_japanese, 2)) {
					...
					isUpdated.put(2, 0);
					bar = new Processbar(tag_japanese.size(), 30);
				} else {
					tags_list.add(tag_temp);
					System.err.println(path_japanese + " read error");
				}
			...
			if (commandMap.containsKey(Command.SPANISH_RESOURCE_FILE)) {
				...
				if (readValueFromExcel(path_spanish, tag_spanish, 3)) {
					...
					isUpdated.put(3, 0);
					bar = new Processbar(tag_spanish.size(), 30);
				} else {
					tags_list.add(tag_temp);
			...
			if (commandMap.containsKey(Command.GERMAN_RESOURCE_FILE)) {
				...
				if (readValueFromExcel(path_german, tag_german, 4)) {
					...
					isUpdated.put(4, 0);
					bar = new Processbar(tag_german.size(), 30);
				} else {
					tags_list.add(tag_temp);
			...
			if (commandMap.containsKey(Command.HANS_RESOURCE_FILE)) {
				...
				if (readValueFromExcel(path_hans, tag_hans, 5)) {
					...
					isUpdated.put(5, 0);
					bar = new Processbar(tag_hans.size(), 30);
				} else {
					tags_list.add(tag_temp);
			...
			if (commandMap.containsKey(Command.HANT_RESOURCE_FILE)) {
				...
				if (readValueFromExcel(path_hant, tag_hant, 6)) {
					...
					isUpdated.put(6, 0);
					bar = new Processbar(tag_hant.size(), 30);
				} else {
					tags_list.add(tag_temp);
			...
			// }
			// System.err.println(tags_list.get(0).size());
			// System.err.println(tag_default_modified.size());
			if (tags_list.size() != 0) {
				// update map
				updateMap();
				// for (Entry<Integer, Integer> entry : isUpdated.entrySet()) {
			...
			if (entry.getValue() == 1) {
				...
				case 1:
					...
					for (Entry<String, String> entry2 : tags_list.get(entry.getKey() - 1).entrySet()) {
						...
						// System.err.println(n + "write");
						bar.showBarByPoint(n);
						n++;
					}
