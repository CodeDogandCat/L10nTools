...
public abstract class AbstractCore {
		...
		if (readValueFromExcel(path_db, sheetName)) {
			...
			for (Map.Entry<String, String> entry : tag_defaultMap.entrySet()) {
				...
				if (tag_otherMap.containsKey(entry.getKey())) {
					...
					String othertValue = tag_otherMap.get(entry.getKey());

					String newValue = "";

					if (tag_default_other.containsKey(defaultValue)) {
						newValue = tag_default_other.get(defaultValue);

						if (!othertValue.equals(newValue)) {

							// System.err.println(otherValue);

							// rewrite to string xml
							if ((rewriteString = setTagValue(entry.getValue(), newValue)) != null) {
	...
	protected void printCheckerBody() {
		System.out.print("#################################################################################" + "\n");
		System.out.print("###############" + "\n");
		System.out.print("###############         value checking result for " + path_other + "\n");
		System.out.print("###############" + "\n");
		System.out.print("#################################################################################" + "\n");
		...
		if (!tag_missMap.isEmpty()) {
			...
			if (rewriteList.size() != 0) {
				System.out.print("                        update  " + rewriteList.size() + " item(s)" + "\n");
				System.out.print("\n\n");
			...
			// System.out.print("                        rewrite  miss " +
			// rewrite_miss_List.size() + " item(s)" + "\n");
			// System.out.print("\n\n");
